...
SETUP = {"first_node_name":"Selle",
		"first_node_color":"#FFFFFF",
		"graph_filename":"input.diag"}
...
#				"default_group_color = '#BBBBBB'\n"
	...
	def fill(self, filename = "input.csv"):
		with open(filename, 'r') as csvfile:
			...
			for row in listing:
				try:
					node = Node(row[4],row[0],row[1],[row[5],row[6],row[7]])
					print(node)
	...
	def export_graph(self, filename = SETUP["graph_filename"]):
...
class Node(object):
	...
	def recursive_list(self, f):
		f.write(self.level + " - "+self.name + "\t" + self.appr[0]+"\t"+ self.appr[1] +"\t"+ self.appr[2] + "\n" )
		for node in self.next_nodes:
	...
	def recursive_hlist(self, f):
		...
		if self.indice != 0:
			f.write(self.level + " - ")
		if self.appr != ['*','*','*']:
			f.write(self.name + "\t"+ self.appr[0]+"\t"+ self.appr[1] + "\t"+ self.appr[2] + "\n")
		else:
			f.write(self.name + '\n')
		for node in self.next_nodes:
